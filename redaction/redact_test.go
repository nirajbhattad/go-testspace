...
// TestNoRedactFieldsJsonBackup tests redaction on a simple struct with json and no redact tags with backup logic
func TestNoRedactFieldsJsonBackup(t *testing.T) {
...
// TestNoRedactFieldsXmlBackup tests redaction on a simple struct with xml and no redact tags with backup logic
func TestNoRedactFieldsXmlBackup(t *testing.T) {
...
// TestRedactSimpleBackup tests redaction on a simple struct with json with backup logic
func TestRedactSimpleBackup(t *testing.T) {

	// Simple struct
	type User struct {
		username      string   `redact:""`
		Password      string   `json:"password" redact:""`
		DbSecrets     []string `json:"dbSecrets"`
		SIN           *int     `json:"sin" redact:""`
		SSN           int      `json:"ssn" redact:""`
...
// TestRedactSimpleXmlBackup tests redaction on a simple struct with xml with backup logic
func TestRedactSimpleXmlBackup(t *testing.T) {
...
// TestNoRedactFieldsXmlBackup tests redaction on a simple struct with xml and no redact tags with backup logic
func TestRedactedLogBackup(t *testing.T) {
	...
	validateBackUpRedaction(t, test, test)
	validateBackUpRedaction(t, string(test), string(test))
}

func TestMarshalFailover(t *testing.T) {
	t.Helper()
	...
	got := MarshalRedactPayload(x, true)
	assert.Equal(t, got, err.Error(),
		"Error mismatch after marshalling fields")
}
...
// validateBackUpRedaction is a helper function to validate redaction functionality with backup logic on any types apart from struct.
func validateBackUpRedaction(t *testing.T, msg, redactedMsg interface{}) {
	t.Helper()
	...
	var b []byte
	switch redactedMsg := redactedMsg.(type) {
	case []byte:
		b = redactedMsg
	case string:
		b = []byte(redactedMsg)
	...
	assert.Equal(t, string(b), got,
		"Backup representation mismatch after redacting fields")
}
